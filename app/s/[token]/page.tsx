...
import {
  ...
  construirVistaDestinatario,
  estadoPermiteDestinatario,
  crearLimitadorDeTasa,
  type VistaComercioTemporal,
  type VistaDestinatarioTemporal,
} from '@/lib/temporary-access'
import { CompartirConClienteButton } from './CompartirConClienteButton'
...
export default async function VistaTemporalComercioPage({
  ...
  // mismo no esté vencido/revocado.
  ...
  // Una sola vez por apertura de vista, y del acceso que se está USANDO
  // (el hijo cuando se abre el link destinatario) — nunca del parent
  // (sección 20: "no debe incrementar contador del parent cuando abre el
  // hijo"), nunca desde el endpoint de evidencia.
  await registrarAccesoVista(acceso.id)

  if (acceso.tipo === 'destinatario') {
    const vista = construirVistaDestinatario(acceso.solicitudId, solicitudData)
    return <VistaDestinatario vista={vista} token={token} />
  }

  const vista = construirVistaComercio(acceso.solicitudId, solicitudData)
  const puedeCompartir = estadoPermiteDestinatario(solicitudData.estado)
  return <VistaComercio vista={vista} token={token} puedeCompartir={puedeCompartir} />
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="max-w-2xl mx-auto flex flex-col gap-4">
        ...
        {/* Sección 18: solo si el token padre es válido (ya lo es, se llegó
            hasta acá) y el estado de la orden lo permite (D5). */}
        {puedeCompartir && (
          <div className="flex justify-center">
            <CompartirConClienteButton token={token} />
          </div>
        )}
...
// ─── Vista destinatario (Bloque 2) ──────────────────────────────────────────
//
// Deliberadamente más simple que VistaComercio (sección 19): sin tarjetas
// financieras, sin recolección del comercio, sin galería general de
// evidencias — solo entrega + foto de entrega + motorizado + timeline.
function VistaDestinatario({ vista, token }: { vista: VistaDestinatarioTemporal; token: string }) {
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="max-w-2xl mx-auto flex flex-col gap-4">
        ...
        <div className="text-center mb-2">
          <p className="text-xl font-black text-[#004aad]">StorkHub</p>
          <p className="text-xs text-gray-400">Seguimiento de tu envío</p>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="px-4 py-3 bg-blue-50 border-b border-blue-100">
            <h2 className="text-xs font-bold uppercase tracking-wide text-[#004aad]">Entrega</h2>
          </div>
          <div className="p-4 space-y-3">
            {vista.fueraManagua ? (
              <div>
                <p className="text-[10px] font-bold text-violet-600 uppercase tracking-wide mb-1">
                  {vista.fueraManagua.metodoEnvio === 'cargotrans' ? 'Sucursal Cargotrans' : 'Terminal / Bus'}
                </p>
                <p className="text-sm font-bold text-gray-900">{vista.fueraManagua.puntoLogisticoNombre || vista.fueraManagua.terminalSugerida || '—'}</p>
                {vista.fueraManagua.destinoFinal && <p className="text-sm text-violet-700 font-semibold">Destino: {vista.fueraManagua.destinoFinal}</p>}
                {vista.fueraManagua.direccionPuntoLogistico && <p className="text-xs text-gray-500 mt-1">{vista.fueraManagua.direccionPuntoLogistico}</p>}
              </div>
            ) : (
              <div>
                <p className="text-sm font-bold text-gray-900">{vista.entrega.nombreApellido || '—'}</p>
                <p className="text-sm text-gray-600">{vista.entrega.direccionEscrita || '—'}</p>
                {vista.entrega.celular && <p className="text-xs text-gray-400 mt-1">{vista.entrega.celular}</p>}
              </div>
            )}
            {vista.entrega.nota && (
              <div className="pt-2 border-t border-gray-100">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Instrucciones</p>
                <p className="text-sm text-gray-600">{vista.entrega.nota}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {vista.evidenciaEntregaDisponible && (
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            <div className="px-4 py-3 bg-green-50 border-b border-green-100">
              <h2 className="text-xs font-bold uppercase tracking-wide text-green-700">Foto de entrega</h2>
            </div>
            <div className="p-4">
              {(() => {
                const src = `/api/access/${token}/evidence/entrega`
                return (
                  <a href={src} target="_blank" rel="noopener noreferrer" className="block group">
                    ...
                    <img
                      src={src}
                      alt="Foto de entrega"
                      className="w-full max-h-72 object-contain rounded-xl border border-green-200 bg-green-50 group-hover:opacity-90 transition"
                      loading="lazy"
                    />
                  </a>
                )
