'use client'
import { useEffect, useState } from 'react'

function Pill({ label, value, color }: { label: string; value: string | null | undefined; color?: string }) {
  const hasValue = !!value
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', minWidth: 0 }}>
      <span style={{ fontSize: 10, color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.4, whiteSpace: 'nowrap' as const }}>
        {label}
      </span>
      <span
        style={{
          fontSize: 13, fontWeight: 700,
          color: hasValue ? (color || '#111827') : '#cbd5e1',
          whiteSpace: 'nowrap' as const, maxWidth: 160,
          overflow: 'hidden', textOverflow: 'ellipsis',
        }}
        title={value || undefined}
      >
        {hasValue ? value : '—'}
      </span>
    </div>
  )
}

function Divider() {
  return <div style={{ width: 1, height: 32, background: '#e5e7eb', flexShrink: 0 }} />
}

function StatusBadge({ formularioCompleto, camposFaltantes }: { formularioCompleto: boolean; camposFaltantes: number }) {
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 4,
      fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 20,
      background: formularioCompleto ? '#f0fdf4' : '#fffbe6',
      color: formularioCompleto ? '#16a34a' : '#d46b08',
      border: `1px solid ${formularioCompleto ? '#bbf7d0' : '#ffe58f'}`,
      whiteSpace: 'nowrap' as const, flexShrink: 0,
    }}>
      <span style={{ width: 5, height: 5, borderRadius: '50%', background: formularioCompleto ? '#16a34a' : '#f59e0b', display: 'inline-block' }} />
      {formularioCompleto ? 'Lista' : camposFaltantes === 1 ? '1 pendiente' : `${camposFaltantes} pendientes`}
    </span>
  )
}

export default function StickyOrderHeader({
  precio, distanciaKm, zonaRetiro, zonaEntrega, retiroNombre, camposFaltantes, formularioCompleto,
}: {
  precio: number | null | undefined
  distanciaKm: number | null | undefined
  zonaRetiro: string | null | undefined
  zonaEntrega: string | null | undefined
  retiroNombre: string | null | undefined
  camposFaltantes: number
  formularioCompleto: boolean
}) {
  const [isMobile, setIsMobile] = useState(false)

  useEffect(() => {
    const mq = window.matchMedia('(max-width: 767px)')
    setIsMobile(mq.matches)
    const h = (e: MediaQueryListEvent) => setIsMobile(e.matches)
    mq.addEventListener('change', h)
    return () => mq.removeEventListener('change', h)
  }, [])

  const precioLabel = precio ? (precio === -1 ? 'Consultar' : `C$ ${precio}`) : null
  const zonaLine = zonaRetiro && zonaEntrega
    ? `${zonaRetiro} → ${zonaEntrega}`
    : zonaRetiro || zonaEntrega || null

  const wrapStyle: React.CSSProperties = {
    position: 'fixed',
    top: isMobile ? 48 : 0,
    left: isMobile ? 0 : 'var(--sidebar-width, 250px)' as any,
    right: 0,
    zIndex: 100,
    background: 'rgba(255,255,255,0.97)',
    backdropFilter: 'blur(8px)',
    borderBottom: '1px solid #e5e7eb',
    boxShadow: '0 2px 8px rgba(0,0,0,0.05)',
    transition: 'left 300ms ease-in-out',
  }

  /* ── Móvil: layout 2 columnas compacto ── */
  if (isMobile) {
    return (
      <div style={wrapStyle}>
        <div style={{ padding: '7px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
          {/* Izquierda: retiro + zonas */}
          <div style={{ flex: 1, minWidth: 0 }}>
            {retiroNombre && (
              <div style={{ fontSize: 13, fontWeight: 700, color: '#111827', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {retiroNombre}
              </div>
            )}
            {zonaLine && (
              <div style={{ fontSize: 11, color: '#6b7280', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 1 }}>
                {zonaLine}
              </div>
            )}
            {!retiroNombre && !zonaLine && (
              <div style={{ fontSize: 12, color: '#9ca3af' }}>Sin datos aún</div>
            )}
          </div>

          {/* Derecha: precio + distancia + badge */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            {precioLabel && (
              <span style={{ fontSize: 15, fontWeight: 900, color: '#004aad' }}>{precioLabel}</span>
            )}
            {distanciaKm && (
              <span style={{ fontSize: 11, color: '#6b7280', fontWeight: 600 }}>{distanciaKm.toFixed(1)} km</span>
            )}
            <StatusBadge formularioCompleto={formularioCompleto} camposFaltantes={camposFaltantes} />
          </div>
        </div>
      </div>
    )
  }

  /* ── Desktop: layout multi-pill horizontal ── */
  return (
    <div style={wrapStyle}>
      <div style={{
        maxWidth: 760, margin: '0 auto', padding: '8px 16px',
        display: 'flex', alignItems: 'center', gap: 12,
        overflowX: 'auto' as const, scrollbarWidth: 'none' as const,
      }}>
        {retiroNombre && (
          <>
            <Pill label="Retiro" value={retiroNombre} color="#111827" />
            <Divider />
          </>
        )}
        <Pill label="Precio" value={precioLabel} color="#004aad" />
        <Pill label="Distancia" value={distanciaKm ? `${distanciaKm.toFixed(1)} km` : null} color="#374151" />
        {(zonaRetiro || zonaEntrega) && (
          <>
            <Divider />
            <Pill label="Ret. zona" value={zonaRetiro} color="#374151" />
            <Pill label="Ent. zona" value={zonaEntrega} color="#374151" />
          </>
        )}
        <div style={{ marginLeft: 'auto', flexShrink: 0 }}>
          <StatusBadge formularioCompleto={formularioCompleto} camposFaltantes={camposFaltantes} />
        </div>
      </div>
    </div>
  )
}
